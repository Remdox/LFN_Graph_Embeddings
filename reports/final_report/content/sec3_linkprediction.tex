...
\begin{itemize}
    \item \textbf{Support Vector Machines} (SVM) \cite{SVM} chosen as linear baseline. 
    \item \textbf{Random Forest} (RF) \cite{RandomForest} chosen for its fast computation.
    \item \textbf{Multilayer Perceptron} (MLP) \cite{MLP} chosen for its accurate predictions.
\end{itemize}