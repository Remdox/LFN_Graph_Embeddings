...
\begin{itemize}
    \item Node2Vec \cite{Node2VecImpl}
    \item LINE \cite{LINEImpl}: the original implementation was optimized for computational efficiency and training stability (for example, using sparse embeddings, SparseAdam optimizer and Batch Matrix Multiplication). 
    ...
    \item DVNE \cite{DVNEImpl}: this implementation has been severely rewritten for adapting it to our link prediction pipeline and to make it more similar to the original paper.
    ...
    \item GraphSage \cite{GraphSageImpl}: the implementation was modified to support a Graph object as input instead of the original text-based format. 
...
\begin{itemize}
    \item SVM
    \item MLP: simple model using just 1 layer with 16 hidden channels, cross entropy loss and Adam optimizer.
    \item RandomForest: for this classifier was used the XGBRFClassifier implementation. 
