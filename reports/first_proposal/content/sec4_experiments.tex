...
edges which are not included in the sample. We employ each model for all embeddings, so that we can compare the impact of each one of them on the prediction of each model. Finally, the metrics will be used to get relevant information about the quality of the embeddings. The results will be analyzed 
to assess how each embedding scales and adapts with different categories and sizes. \\ \newline
The implementations used are: \cite{Node2VecImpl} for Node2Vec, \cite{LINEImpl} for LINE, \cite{DVNEImpl} for DVNE, \cite{GraphSageImpl} for GraphSage. \\ \newline
Hardware: Colab is used. Specifics of its basic runtime: Intel(R) Xeon(R) CPU 2-core @ 2.20GHz, 13 Gb, 110 Gb disk, Nvidia Tesla T4 (16 Gb VRAM) \\ \newline
